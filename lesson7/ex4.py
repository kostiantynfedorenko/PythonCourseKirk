'''
4. Take the YAML file and corresponding data structure that you defined in
 exercise3b:
{'interfaces': {
...
The following should all be variables in your Jinja template (the names may be
 different than below, but they should be variabilized and not be hard-coded
  in your template).
interface_name
switchport_mode
access_vlan
...
yfile = 'interfaces3b.yml'
tfile = 'interface_template.j2'

with open(yfile) as f:
    interface_data = yaml.load(f)

with open(tfile) as f:
    interface_template = f.read()

t = jinja2.Template(interface_template)
print(t.render(interface_data))

...
Loop over this data structure and extract all of the MAC addresses.
 ...
 Print all of the new standardized MAC address to the screen. The
 standardized format should be as follows:

00:62:EC:29:70:FE

The hex digits should be capitalized. Additionally, there should be a
colon between each octet in the MAC address.
'''
from __future__ import unicode_literals, print_function


def convert_mac(mac):
    # create uppercase MAC without separators
    mac = mac.split('.')
    mac = ''.join(mac).upper()

    # create new MAC with ':' separators
    new_mac = []
    while len(mac) > 0:
        new_mac.append(mac[:2])
        mac = mac[2:]
    return ':'.join(new_mac)
